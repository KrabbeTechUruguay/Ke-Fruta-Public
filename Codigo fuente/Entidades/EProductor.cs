...
namespace Entidades
{
    public class EProductor : EUsuario
    {
        private string ci;
        ...
        public string Rut
        {
            ...
            set
            {
                ...
                {
                    throw new Exception("Dato incorrecto para rut");
                }
                else
                {
                    rut = value;
        ...
        public string Produce
        {
            ...
            set
            {
                if (string.IsNullOrEmpty(value))
                {
                    throw new Exception("Debe seleccionar que produce");
                }
                else
                {
                    produce = value.ToUpper();
                    if (produce == "FRUTA" || produce == "FRUTAS" || produce == "FRUT")
                    {
                        produce = "FRU";
                    }
                    else if (produce == "CEREAL" || produce == "CEREALES" || produce == "CERE")
                    {
                        produce = "CER";
                    }
                    else if (produce == "VERDURA" || produce == "VERDURAS" || produce == "VERDU")
                    {
                        produce = "VER";
                    }
                }
        ...
        public EProductor(string ci, string nom_a, string nom_b, string ape_a, string ape_b, List<string> tels, string mail, string user, string pass, string calle_a, string calle_b, string nro_puerta, bool baja, string rut, string produce) : base(ci, nom_a, nom_b, ape_a, ape_b, tels, mail, user, pass, calle_a, calle_b, nro_puerta, baja)
        {
            this.Ci = ci;
            this.PrimerNombre = nom_a;
            this.SegundoNombre = nom_b;
            this.PrimerApellido = ape_a;
            this.SegundoApellido = ape_b;
        ...
        //CONSTRUCTOR INCOMPLOETO PARA PODER INSTANCIAR USUARIOS SIN ROL AUN ASIGNADO (COSA QUE NO DEBERIA SUCEDER)
        //OJO CUANDO SE INSTANCIE, EN ESTE CONSTRUCTOR ES UN USUARIO QUE EN REALIDAD NO ES PRODUCTOR, SOLO ES USUARIO 
        public EProductor(string ci, string nom_a, string nom_b, string ape_a, string ape_b, List<string> tels, string mail, string user, string pass, string calle_a, string calle_b, string nro_puerta, bool baja) : base(ci, nom_a, nom_b, ape_a, ape_b, tels, mail, user, pass, calle_a, calle_b, nro_puerta, baja)
        {
            this.Ci = ci;
            this.PrimerNombre = nom_a;
            this.SegundoNombre = nom_b;
        ...
        {
            string produceStr = null; 
            if (produce == "FRU") { produceStr = "Fruta"; }
            else if (produce == "CER") { produceStr = "Cereal"; }
            else if (produce == "VER") { produceStr = "Verdura"; }
            return
                (
                "Nombre completo: " + primerNombre + " " + segundoNombre + " " + primerApellido + " " + segundoApellido + System.Environment.NewLine +
                "Productor de: " + produceStr + System.Environment.NewLine + 
                "Cedula: " + ci + System.Environment.NewLine +
                "RUT asociado: " + rut + System.Environment.NewLine +
